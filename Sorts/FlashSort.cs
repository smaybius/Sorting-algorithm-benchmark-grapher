    ...
    internal sealed class FlashSort : IIntegerSorter
    {
        ...
        public void RunSort(ArrayInt[] array, int sortLength, int parameter, IComparer<ArrayInt> cmp)
        {
            ...
            for (int i = 1; i < sortLength - 1; i += 2)
            {
                ...
                if (cmp.Compare(array[i], array[i + 1]) == -1)
                {
                    small = array[i];
                    big = array[i + 1];
                    bigIndex = i + 1;
            ...
            //do the last element
            if (cmp.Compare(array[sortLength - 1], min) == -1)
            {
                min = array[sortLength - 1];
            }
            else if (cmp.Compare(array[sortLength - 1], max) == 1)
            {
            ...
            while (numMoves < sortLength)
            {
                ...
                while (j < L[K])
                {
                    ...
                    (evicted, array[location]) = (array[location], evicted);
            ...
            for (K = m - 1; K >= 1; K--)
            {
                ...
                if (classSize > threshold && classSize > minElements)
                {
                    ...
                    ArrayInt[] copiedRange = Sort.CopyOfRange(array, L[K], L[K + 1]);
                    RunSort(copiedRange, classSize, 0, cmp);
                }
            }
            InsertionSort.InsertSort(array, 0, sortLength, cmp);
        }
    }
}
